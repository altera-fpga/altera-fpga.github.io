...
# HPS PCIe Root Port System Example Design for the Agilex™ 7 FPGA M-Series Development Kit – HBM2e
...
This reference design demonstrates a PCIe root port running on Agilex™ 7 FPGA M-Series FPGA M-Series Development Kit – HBM2e connected to end point. A Gen5x4 link is shown. The root port reference design is based on the Agilex 7 Golden System Reference Design, with PCIe root port and necessary Linux software infrastructure added.
...
* [Agilex ® 7 FPGA M-Series Development Kit – HBM2e](https://www.altera.com/products/devkit/po-3207/agilex-7-fpga-m-series-development-kit-hbm2e-edition-3x-f-tile-1x-r-tile)
...
* [R-Tile Avalon ® Streaming IP for PCI Express](https://docs.altera.com/r/docs/683501/26.1/r-tile-avalon-streaming-ip-for-pci-express-user-guide/about-the-r-tile-avalon-streaming-ip-for-pci-express)
...
The pre-build binaries [Agilex-7_M-series_Rtile_artifacts.zip](https://github.com/altera-fpga/agilex7-ed-pcie-rp/releases/download/26.1/Agilex-7_M-series_Rtile_artifacts.zip) contain the files below.
...
|----------------|--------------|-----------------|-----------------|
|0x90000000      |256M          |BAS              |	Avalon MM Slave of MCDMA BAS port (28-bit address space)|
...
|0xF9000200      |16            |CCT              |	Cache Coherent Translator |
|0xF9010000      |16k           |CS               |Avalon MM Slave of PCIe Configuration Slave port (14b address space)|
...
|0xF90180C0      |1k           |AVMM CS Cpl TimeOut & System level Reg. map |Error registers along with Timeout values |
...
```bash
cd $TOP_FOLDER/src/hw/ag7m039_devkit/syn/
make all
```
...
. agilex7_dk_dev_agm039fes-rped-build.sh
build_setup
```
...
```bash
cd $TOP_FOLDER/src/hw/ag7m039_devkit/syn/
quartus_pfg -c -o hps=on -o hps_path=../../../sw/agilex7_dk_dev_agm039fes-rped-rootfs/tmp/deploy/images/agilex7_dk_dev_agm039fes/u-boot-spl-dtb.hex output_files/top.sof output_files/top.rbf
```
...
Write the $TOP_FOLDER/src/sw/agilex7_dk_dev_agm039fes-rped-images/gsrd-console-image-agilex7.wic. SD card image to the micro SD card using the included USB writer in the host computer:
...
Program the development kit with $TOP_FOLDER/src/hw/ag7m039_devkit/syn/output_files/top.hps.rbf file.
...
```bash
        root@agilex7dkdevagm039fes:~# lspci
```
