...
export ARCH=arm64
export CROSS_COMPILE=aarch64-none-linux-gn
```
...
CONFIG_BOOTCOMMAND="run mmcfitload; run mmcfitboot"
CONFIG_CMD_FAT=y
...
- From Configuration Menu, set the HPS/FPGA configuration order to: After INIT_DONE.
...
quartus_pfg -c ghrd.sof ghrd.rbf -o hps_path=../u-boot-socfpga/spl/u-boot-spl-dtb.hex -o sign_later=ON
```
After that, an .rbf file is created:

- $TOP_FOLDER /bitstreams/ghrd.rbf

Sign the rbf. 
```bash 
quartus_sign --family=agilex7 --operation=sign --qky=../keys/qky/sign0_cancel1.qky --pem=../keys/privatekeys/private_sign0.pem \
ghrd.core.rbf signed_bitstream.rbf
```
...
```
SDM will authenticate the bitstream and only allow configuration to proceed if the image is authenticated. Once RBF programming is done, you will be able to observe the U-boot boot messages at the serial terminal. The FSBL will verify the SSBL. The SSBL will then boot and verify the Linux kernel. 
```bash 
...
NOTICE:  BL31: v2.13.0(release):QPDS26.1.1_REL_GSRD_PR
