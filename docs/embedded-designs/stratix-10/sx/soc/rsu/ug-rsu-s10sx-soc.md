...
Refer to [Stratix® 10 Hard Processor System Remote System Update User Guide](https://www.intel.com/content/www/us/en/docs/programmable/683021/current/overview-s10-fm.html) for details about the Remote System Update. 
...
The end results of the build flow are listed next.
...
Download the compiler toolchain, add it to the PATH variable, to be used by the Quartus design makefile to build the HPS Debug FSBL:
...
Create a Quartus project, based on the Quartus design provided as part of the HPS Baseline System Example Design from GitHub with a few changes listed next. 
...
sed -i '/STRATIX_JTAG_USER_CODE 4/i set_global_assignment -name RSU_MAX_RETRY_COUNT 3' s10_soc_devkit_ghrd/create_ghrd_quartus.tcl
# update sysid to make binaries slightly different 
sed -i 's/0xACD5CAFE/0xABAB1234/g' s10_soc_devkit_ghrd/create_ghrd_qsys.tcl
...
After completing the above steps, the following SOF file is created are listed next. 
...
<pfg version="1">
    ...
    <bitstreams>
        <bitstream id="Bitstream_1">
            <path signing="OFF" finalize_encryption="0" hps_path="./u-boot-socfpga/spl/u-boot-spl-dtb.hex">./stratix10-ed-gsrd/install/designs/s10_htile_soc_devkit_oobe_baseline.sof</path>
        </bitstream>
...
9. In the **Input Files** tab click the **Add Bitstream** button, then browse to **$TOP_FOLDER/stratix10-ed-gsrd/output_files**, select the file **s10_htile_soc_devkit_oobe_baseline.sof**, and then click **Open**. This is the initial factory image. Do the same for the **$TOP_FOLDER/stratix10-ed-gsrd/install/designs/s10_htile_soc_devkit_oobe_baseline.sof** image. This is the initial application image. The tab now looks like below. 
...
17. Select the **MT25QU02G** flash device in the Configuration Device tab by clicking it, then click the **Add Partition** button to open the **Add Partition** window. Leave the **Name** as **P1** and select the **Input file** as **Bitstream_2(s10_htile_soc_devkit_oobe_baseline.sof)**. This becomes the initial application image. Select the **Address Mode** as **Block** and allocate 16MB of data by setting **Start Address** = **0x01000000** and **End Address** = **0x01FFFFFF**. Since this is the first partition defined, this becomes the initial application image to be loaded and has the highest priority of all application images that may be defined later.
...
-o app_image=stratix10-ed-gsrd/install/designs/s10_htile_soc_devkit_oobe_baseline.sof \
-o hps_path=u-boot-socfpga/spl/u-boot-spl-dtb.hex \
...
3. Configure the Stratix 10 SoC Development Kit as follows, to have MSEL set to JTAG:
    ...
    - SW1: 1:OFF, rest:ON <br>
    - SW2: 1:ON 2:ON 3: ON 4:N/A (SW2.4 is is not connected) <br>
    - SW3: all OFF <br>
    - SW4: 1:ON 2:OFF 3:OFF 4:ON <br>
...
12. Load the newly requested image. Stop at U-Boot prompt and check the status log 
    to confirm it: 
    ```bash 
    SOCFPGA # rsu status_log 
    Current Image : 0x01000000 
