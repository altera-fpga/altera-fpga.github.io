...
# SoC FPGA Remote Debug Tutorial Example Design: Agilex™ 3 FPGA and SoC C-Series Development Kit
...
The example is based on the [GSRD](https://altera-fpga.github.io/rel-25.1.1/embedded-designs/agilex-5/e-series/premium/gsrd/ug-gsrd-agx5e-premium/), with the following changes:
...
```bash
sudo rm -rf agilex3.remote_debug
...
```bash
cd $TOP_FOLDER/agilex3_soc_devkit_ghrd
...
```patch
diff --git a/recipes-bsp/device-tree/files/socfpga_agilex3_ghrd.dtsi b/recipes-bsp/device-tree/files/socfpga_agilex3_ghrd.dtsi
...
The instructions from this section present how to run the remote debug example. Refer to the [GSRD](https://altera-fpga.github.io/rel-25.1.1/embedded-designs/agilex-5/e-series/premium/gsrd/ug-gsrd-agx5e-premium/) for more detailed instructions on how to set up the board, serial port, and write the binaries.

1\. Write the QSPI image `$TOP_FOLDER/ghrd_a3cw135bm16ae6s.hps.jic` to flash.

2\. Extract and write the SD card image `$TOP_FOLDER/gsrd-socfpga/agilex3-gsrd-images/sdimage.tar.gz` to the SD card
...
```bash
root@agilex3:~# rmmod uio_pdrv_genirq
...
```bash
root@agilex3:~# etherlink --port=33301
