...
<h5> User Flow 2: Testing Complete Flow. </h5>
 ...
 ||[Run Iperf Test](#running-iperf-test)|Yes|Yes|
 ||[Run Time Synchronization](#run-time-synchronization)|Yes|Yes|
...
git clone -b rel/24.3 https://github.com/altera-fpga/agilex5-ed-tsn-sgmii.git
...
# Note : If user doing compilation first time, download the prebuilt u-boot-spl-dtb.hex  file and create the following path $TOP_FOLDER/agilex5-ed-tsn-sgmii/src/sw/agilex5_dk_a5e065bb32aes1-gsrd-images/u-boot-agilex5-socdk-gsrd-atf/ and copy the u-boot-spl-dtb.hex file here.
...
* Download the [Prebuild Binaries](https://github.com/altera-fpga/agilex5-ed-tsn-sgmii/releases/tag/24.3-1) into this path $TOP_FOLDER, if you are leveraging on _[User Flow 1](#user-flow-1-testing-with-prebuild-binaries)._
...
- Board A (as Master):
   ...
   Run the below command and get the `eth0` and system clock time.

    ```bash
    phc_ctl /dev/ptp0 get && phc_ctl CLOCK_REALTIME get
    ```
   Run the below command and sync realtime system clock time to `eth0` node time. 

    ```bash
    phc2sys -s CLOCK_REALTIME -c /dev/ptp0 -r -O 0 -R 16 -m 
    ```
    Run `ptp4l` command with end to end delay mechanism option.
...
- At Board B (as slave), perform sync on local System Clock with EMAC Hardwware Clock.
    ...
    ```bash
     ptp4l -i eth0  -s -H -E -2 -m & phc2sys -s eth0 -w -m -c CLOCK_REALTIME -O 0 -n 0
    ```
...
- Board A (as Master):
   ...
   Run the below command and sync realtime system clock time to `eth0` node time. 
    ...
    Run `ptp4l` command with peer to peer delay mechanism option.
    ```bash
...
- At Board B (as slave), perform sync on local System Clock with EMAC Hardwware Clock.
    ...
    ```bash
    ptp4l -i eth0  -s -H -P -2 -m & phc2sys -s eth0 -w -m -c CLOCK_REALTIME -O 0 -n 0
    ```
...
- Board A (as master):
   ...
   Run the below command and sync realtime system clock time to `eth0` node time. 
    ...
    phc2sys -s CLOCK_REALTIME -c /dev/ptp0 -r -O 0 -R 16 -m 
    ```
    Run `ptp4l` command with transportSpecific value 1.
...
- At Board B (as slave), perform sync on local System Clock with EMAC Hardwware Clock.
    ...
    ```bash
    ptp4l -i eth0  -s -H -P -2 -m --transportSpecific=1 & phc2sys -s eth0 -w -m --transportSpecific 1 -c CLOCK_REALTIME -O 0 -n 0
    ```
